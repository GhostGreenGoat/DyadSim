...
class LLMClient:
    ...
    def _call_deepseek(
        ...
        if not self.api_keys["deepseek"]:
            # 如果没有DeepSeek API密钥，直接回退到OpenRouter
            print("未找到DeepSeek API密钥，回退到OpenRouter的GPT-4.1")
            return self._fallback_to_openrouter_gpt4(prompt, temperature, max_tokens, system_prompt, messages)
        ...
        try:
            ...
            print(f"调用DeepSeek API连接错误: {str(e)}")
            # 对于连接错误，回退到OpenRouter GPT-4.1
            print("DeepSeek API连接错误，回退到OpenRouter的GPT-4.1")
            return self._fallback_to_openrouter_gpt4(prompt, temperature, max_tokens, system_prompt, messages)
        except Exception as e:
            print(f"调用DeepSeek API出错: {str(e)}")
            # 对于其他错误，检查是否是连接相关的异常
            if "Connection reset" in str(e) or "Connection aborted" in str(e) or "timeout" in str(e).lower():
                print("DeepSeek API连接问题，回退到OpenRouter的GPT-4.1")
                return self._fallback_to_openrouter_gpt4(prompt, temperature, max_tokens, system_prompt, messages)
            # 其他错误直接返回错误信息
            fallback_info = {"error": str(e), "fallback": False}
            return f"API调用失败: {str(e)}", fallback_info
            
    def _fallback_to_openrouter_gpt4(
        self, 
        prompt: str, 
        temperature: float,
        max_tokens: int,
        ...
        """从DeepSeek回退到OpenRouter的GPT-4.1"""
        try:
            print("尝试回退到OpenRouter的GPT-4.1...")
            # 临时保存当前API类型
            ...
            self.api_base = OPENROUTER_API_BASE
            gpt4_model = "openai/gpt-4.1"  # 使用GPT-4.1模型
            
            # 调用OpenRouter API，指定使用GPT-4.1模型
            result = self._call_openrouter(prompt, gpt4_model, temperature, max_tokens, system_prompt, messages)
            ...
            print(f"回退到OpenRouter GPT-4.1也失败: {str(fallback_error)}")
    ...
    def _call_openrouter(
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                print(f"请求模型: {model or self.default_model}")
