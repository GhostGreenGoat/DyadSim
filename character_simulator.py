...
from api.data.prompt_templates import (
    ...
    build_role_play_prompt
)

# 加载全局profile库
with open(os.path.join(os.path.dirname(__file__), 'api', 'data', 'extracted_character_profiles.json'), 'r', encoding='utf-8') as f:
    _profile_db = {p['id']: p for p in json.load(f)["character_profiles"]}

def get_profile_by_id(profile_id):
    return _profile_db.get(profile_id)
...
class CharacterSimulator:
    ...
    def __init__(
        ...
        partner_profile: Optional[Dict[str, Any]] = None, # 新增
        partner_situation_id: Optional[str] = None, # 新增
        ...
        num_experts: int = 3,
        mode: str = "default", # 新增mode参数
        initial_emotion: str = None, # 新增
        initial_emotion_score: int = None # 新增
    ):
        """
        初始化虚拟人物模拟器
        ...
        参数:
            ...
            partner_profile (Dict[str, Any], optional): 对话伴侣配置
            partner_situation_id (str, optional): 对话伴侣情境ID
            ...
            mode (str): 模式，默认为"default"
            initial_emotion (str, optional): 初始情绪名
            initial_emotion_score (int, optional): 初始情绪分数
        """
        # 初始化API客户端
        ...
        if character_config and scenario_id:
            ...
            if partner_profile is not None and partner_situation_id is not None:
                self.partner_profile = partner_profile
                self.partner_situation = get_situation_by_id(scenario_id, partner_situation_id)
            else:
                partner_profile_id = f"{scenario_id}_{scenario['situations'][1]['id']}" if len(scenario['situations']) > 1 else None
                self.partner_profile = get_profile_by_id(partner_profile_id) if partner_profile_id else None
                self.partner_situation = scenario['situations'][1] if len(scenario['situations']) > 1 else situation
        else:
            # 随机选择一个场景和两个角色
            scenario = random.choice(conflict_scenarios)
            situations = scenario["situations"]
            if len(situations) < 2:
                raise ValueError(f"场景 {scenario['id']} 不足2个情境，无法分配主角和伴侣")
            situation0, situation1 = situations[0], situations[1]
            self.scenario = {
                "scenario": scenario,
                "situation": situation0
            }
            char_profile_id = f"{scenario['id']}_{situation0['id']}"
            partner_profile_id = f"{scenario['id']}_{situation1['id']}"
            self.character = get_profile_by_id(char_profile_id)
            self.partner_profile = get_profile_by_id(partner_profile_id)
            self.partner_situation = situation1
            if not self.character or not self.partner_profile:
                raise ValueError(f"未找到主角或伴侣profile: {char_profile_id}, {partner_profile_id}")
        ...
        # 设置初始情绪值，确保在合理范围内
        if initial_emotion_score is not None:
            self.current_emotion_score = initial_emotion_score
        else:
            self.current_emotion_score = max(
                min(self._calculate_initial_emotion(), 
        ...
        self.initial_emotion_score = self.current_emotion_score
    ...
    def _calculate_initial_emotion(self, return_both=False):
        """
        根据场景和角色特征计算初始情绪值
        返回:
            int: 初始情绪值，范围在临界阈值和改善阈值之间
            或 (情绪名, 分数) if return_both=True
        """
        critical_threshold = emotion_scoring["threshold"]["critical"]
        improvement_threshold = emotion_scoring["threshold"]["improvement"]
        min_emotion = critical_threshold + 3
        max_emotion = improvement_threshold - 1
        ...
        if base_emotion <= -5:
            ...
            emotion_name = "积极"
        if return_both:
            return emotion_name, base_emotion
        return base_emotion
    ...
    def _prepare_prompts(self):
        """准备角色和伴侣的提示词，主角和伴侣profile分开"""
        try:
            # 主角profile
            profile = self.character
            situation = self.scenario["situation"]
            self.character_prompt = build_role_play_prompt(profile, situation, mode=self.mode)
            # 伴侣profile和situation优先用传入的
            partner_profile = self.partner_profile
            partner_situation = self.partner_situation
            self.partner_prompt = build_role_play_prompt(partner_profile, partner_situation, mode=self.mode) if partner_profile and partner_situation else ""
            self._log_prompts()
    ...
    def _analyze_with_experts(self) -> List[Dict[str, Any]]:
        ...
        for i in range(self.num_experts):
            ...
            while retry_count < max_retries:
                try:
                    ...
                    # 添加超时控制
                    expert_response, response_data = expert_client.call(
                        prompt=expert_prompt,
                        temperature=0.2 + (i * 0.1),  # 每个专家使用略微不同的温度
                        max_tokens=500,  # 限制生成长度以加快响应
                        system_prompt="你是一个专业的情感分析专家。请分析给定对话中角色的情绪状态，并提供JSON格式的分析结果。"
                    )
                    
                    # 检查是否使用了回退机制
                    if isinstance(response_data, dict) and response_data.get("fallback", False):
                        print(f"专家 {i+1} 使用了API回退机制")
                    ...
                    try:
                        ...
                        if json_match:
                            ...
                            # 如果没有找到标准JSON，尝试基于关键词提取
                            print(f"专家 {i+1} 未返回标准JSON格式，尝试基于关键词提取")
                            expert_json = {
                        ...
                        # 验证提取的字段
                        if not expert_json.get("primary_emotion") or expert_json.get("primary_emotion") == "未知":
                            print(f"专家 {i+1} 未能提取有效的主要情绪，尝试使用正则表达式直接从文本中提取")
                            # 使用更宽松的正则表达式
                            primary_emotion = re.search(r'情绪[是为:：]\s*["\'"]?([^"\',，。；;]+)["\'"]?', expert_response)
                            if primary_emotion:
                                expert_json["primary_emotion"] = primary_emotion.group(1).strip()
                        
                        if not expert_json.get("intensity"):
                            # 默认强度为中等(3)
                            expert_json["intensity"] = 3
                            
                        if not expert_json.get("emotion_score"):
                            # 根据情绪类型估算分数
                            # 负面情绪
                            if expert_json.get("primary_emotion") in ["悲伤", "愤怒", "失望", "烦躁", "焦虑", "恐惧"]:
                                expert_json["emotion_score"] = -6
                            # 正面情绪
                            elif expert_json.get("primary_emotion") in ["高兴", "快乐", "期待", "满足", "欣慰"]:
                                expert_json["emotion_score"] = 5
                            # 中性情绪
                            else:
                                expert_json["emotion_score"] = 0
                        ...
                        print(f"解析专家 {i+1} 分析JSON失败: {str(json_error)}")
                        print(f"原始响应: {expert_response[:200]}...") # 只打印前200个字符避免日志过长
                        
                        if retry_count < max_retries - 1:
                            print(f"尝试重新获取专家 {i+1} 的分析结果...")
                            retry_count += 1
                            continue
                    ...
                    print(f"专家 {i+1} API调用失败: {str(api_error)}")
                    
                    if retry_count < max_retries - 1:
                        # 如果是连接错误，先尝试另一个API
                        if "Connection reset" in str(api_error) or "Connection aborted" in str(api_error):
                            print(f"尝试使用不同的API进行专家 {i+1} 分析...")
                            # 切换API类型
                            if expert_api == "deepseek":
                                expert_api = "openrouter"
                            else:
                                expert_api = "deepseek"
                            retry_count += 1
                            continue
        ...
        # 如果没有成功的分析，使用估计的情绪值
        if successful_analyses == 0 and expert_analyses:
            print("所有专家分析均失败，使用估计值")
            # 添加一个基于对话内容和角色特性的估计分析
            estimated_analysis = self._estimate_emotion_from_dialogue()
            if estimated_analysis:
                estimated_analysis["expert_id"] = "estimated"
                estimated_analysis["turn"] = self.turn_count
                estimated_analysis["timestamp"] = time.strftime("%Y-%m-%d %H:%M:%S")
                expert_analyses.append(estimated_analysis)
        
        return expert_analyses
        
    def _estimate_emotion_from_dialogue(self) -> Dict[str, Any]:
        """
        当所有专家分析失败时，基于对话内容估计情绪状态
        
        返回:
            Dict[str, Any]: 估计的情绪分析结果
        """
        ...
        # 获取最后一轮角色的消息
        last_message = self.dialogue_history[-1].get("character_message", "")
        
        # 定义情绪关键词及其对应的评分和强度
        emotion_keywords = {
            "高兴": {"score": 5, "intensity": 4, "keywords": ["谢谢", "开心", "喜欢", "爱你", "感谢", "好的"]},
            "平静": {"score": 0, "intensity": 2, "keywords": ["嗯", "好吧", "我知道了", "明白"]},
            "失望": {"score": -4, "intensity": 3, "keywords": ["失望", "算了", "随便", "不用了", "无所谓"]},
            "生气": {"score": -7, "intensity": 5, "keywords": ["气死了", "烦", "讨厌", "滚", "不想"]},
            "悲伤": {"score": -6, "intensity": 4, "keywords": ["难过", "伤心", "痛苦", "难受", "哭"]},
            "焦虑": {"score": -5, "intensity": 4, "keywords": ["担心", "害怕", "怕", "紧张", "忧虑"]}
        }
        ...
        if not emotion_counts:
            if "？" in last_message or "?" in last_message:
                ...
                if self.emotion_history:
                    ...
                    return {
                        "primary_emotion": last_emotion.get("emotion_info", {}).get("emotions", ["平静"])[0],
                        "intensity": 3,
                        "emotion_score": last_emotion.get("score", 0)
                    }
                # 否则默认为平静
                return {
                    "primary_emotion": "平静",
        ...
        return {
            ...
            "intensity": emotion_keywords[primary_emotion]["intensity"],
            "emotion_score": emotion_keywords[primary_emotion]["score"]
        }
...
"""
    ...
    def simulate_turn(self) -> Dict[str, Any]:
        ...
        #===============================================
        # 新增：打印每轮对话前的prompt
        # print("\n[Prompt] 当前轮次角色(character)收到的prompt:")
        # for msg in character_messages:
        #     print(f"  {msg['role']}: {msg['content']}")
        # print("\n[Prompt] 当前轮次伴侣(partner)收到的prompt:")
    ...
    def run_simulation(self, modes=None) -> dict:
        """
        运行完整的对话模拟，支持双模式（default+improved）对比
        modes: None/单一字符串/字符串列表
        返回: 单模式dict或多模式dict
        """
        if modes is None:
            # 兼容旧用法
        ...
        for mode in modes:
            ...
            self.dialogue_history = []
            self.emotion_history = []
            self.emotion_prediction_history = []
            self.expert_analysis_history = []
            self.current_emotion_score = self._calculate_initial_emotion()
            self._prepare_prompts()
            print(f"\n==== 开始{mode}模式模拟 ====")
    ...
    def _save_dialogue_log(self, dialogue_appraisal=None):
        ...
        log_file = os.path.join(
            self.log_dir, 
            f"{self.character['id']}_{self.scenario['situation']['id']}_{self.mode}_{int(time.time())}.json"
        )
